...
class Embedding_based(nn.Module):
    ...
    def calc_kg_loss_TransR(self, h, r, pos_t, neg_t):
        ...
        pos_score = torch.norm(r_mul_h + r_embed - r_mul_pos_t, p=2, dim=1, keepdim=False)  # (kg_batch_size)
        neg_score = torch.norm(r_mul_h + r_embed - r_mul_neg_t, p=2, dim=1, keepdim=False)  # (kg_batch_size)
    ...
    def calc_kg_loss_TransE(self, h, r, pos_t, neg_t):
        ...
        pos_score = torch.norm(h_embed + r_embed - pos_t_embed, p=2, dim=1, keepdim=False)  # (kg_batch_size)
        neg_score = torch.norm(h_embed + r_embed - neg_t_embed, p=2, dim=1, keepdim=False)  # (kg_batch_size)
    ...
    def calc_cf_loss(self, user_ids, item_pos_ids, item_neg_ids):
        ...
        # # 直接将物品嵌入和实体嵌入拼接
        # item_pos_cf_embed = torch.cat((item_pos_embed, item_pos_kg_embed), dim=1)        # (cf_batch_size, 2*embed_dim)
        # item_neg_cf_embed = torch.cat((item_neg_embed, item_neg_kg_embed), dim=1)        # (cf_batch_size, 2*embed_dim)
    ...
    def calc_score(self, user_ids, item_ids):
        ...
        # item_cf_embed = item_embed * item_kg_embed                                      # (n_items, embed_dim)
        # item_cf_embed = torch.cat((item_embed, item_kg_embed), dim=1)                   # (n_items, 2*embed_dim)
